...
def _trending_topics(window_msgs: List[Dict[str, Any]], anchor: datetime) -> List[str]:
    ...
    for m in window_msgs:
        mult = 1.0
        if m["_sentiment_label"] == "positive":
            mult = 1.2
        elif m["_sentiment_label"] == "negative":
            mult = 0.8
        ...
        for h in m.get("hashtags", []):
            ...
            base = 1 + 1 / delta_min
            if len(tag) > 8:
                base *= math.log10(len(tag)) / math.log10(8)
            peso = base * mult
...
def analyze_feed(messages: List[Dict[str, Any]], time_window_minutes: int, now_utc: datetime) -> Dict[str, Any]:
    ...
    engagement_score = round(
        sum(m.get("reactions", 0) + m.get("shares", 0) for m in window_msgs)
        / max(sum(m.get("views", 0) for m in window_msgs), 1)
        * 10,
        2,
    )
